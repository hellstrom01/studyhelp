"""LLM tutor engine using the Claude API.

The study tutor teaches Socratically and writes no reviews (ADR-0004). Every LLM
interaction is a pure prompt-builder + thin API wrapper + pure parser; only the
builders and parsers are unit-tested. Also holds the pure builders/parsers for
the end-of-session summary, subject-memory rewrite, card drafts, and item
classification.
"""
